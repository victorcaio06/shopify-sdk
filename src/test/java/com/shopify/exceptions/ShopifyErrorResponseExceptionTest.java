...
public class ShopifyErrorResponseExceptionTest {
	...
	@Test
	public void testShopifyErrorResponseExceptionWith422StatusCode() {
		...
		final ShopifyErrorResponseException actualException = new ShopifyErrorResponseException(response);
	
		// Verifying the exception details
		assertEquals(
				String.format(ShopifyErrorResponseException.MESSAGE, 422, responseHeaders, expectedResponseBodyString),
				actualException.getMessage());
		assertTrue(actualException instanceof RuntimeException);
		assertEquals(1, actualException.getShopifyErrorCodes().size());
		assertEquals(Type.UNKNOWN, actualException.getShopifyErrorCodes().get(0).getType());
		assertEquals(expectedResponseBodyString, actualException.getShopifyErrorCodes().get(0).getMessage());
		assertEquals(422, actualException.getStatusCode());
		assertEquals(expectedResponseBodyString, actualException.getResponseBody());
	}
